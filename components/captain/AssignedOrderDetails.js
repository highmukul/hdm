...
const AssignedOrderDetails = ({ order, onClose }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-sm sticky top-8">
            ...
            {isLoaded && (
                <GoogleMap mapContainerStyle={containerStyle} center={captainLocation || order?.shippingAddress?.location} zoom={14}>
                    {captainLocation && <Marker position={captainLocation} />}
                    {order?.shippingAddress?.location && <Marker position={order.shippingAddress.location} />}
