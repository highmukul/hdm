...
const ProductCard = ({ product }) => {
  ...
  // The salePrice is now the single source of truth for the price.
      ...
      <Link href={`/products/${product.id}`} className="block text-left flex-grow">
          <div className="relative w-full h-48">
            ...
            {product.stock > 0 && product.stock < 5 && (
              <div className="absolute bottom-3 right-3 bg-yellow-400 text-yellow-900 text-xs font-semibold px-2.5 py-1 rounded-full flex items-center shadow-md">
                  <FiIcons.FiAlertTriangle className="mr-1" /> Only {product.stock} left!
              </div>
            )}
            {product.stock === 0 && (
